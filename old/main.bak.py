...
class KDTree(object):
    ...
    def form_up(self, data, shape, dimension):
        med = np.divide(shape[dimension],2)
        bin = np.where((data[:,dimension] >  med))
        bin = data[bin]
        shape[dimension] = med
        print bin, shape, dimension
        if shape[0] % 2 == 0 and shape[1] % 2 == 0:
            self.form_down(bin,shape,dimension)
            self.form_up(bin, shape, dimension)
        return bin, shape, dimension

    def form_down(self, data, shape, dimension):
        med = np.divide(shape[dimension],2)
        bin = np.where((data[:,dimension] <= med))
        dimension += 1
        if dimension > 1:
